...
function FeaturesComp({spot, loginAccount}) {
    ...
    const features = [
        {
            ...
            activities: ['Food', 'Hotel', 'Hiking', 'Games']
...
const places = [
    ...
    //switch the 'features' to 'categoricFeatures' recieve data from server
    const [categoricFeatures, setCategoricFeatures] = useState([]);
    ...
    //The requested catergory must match the catergory and location from the database before it renders the data that matches
    
    //resp.data.activities.includes(spot) ? setCategoricFeatures(resp.data) : null;
    //     })
    //     .catch(err => {
    //         console.log(err)
    //     })
  ...
  return (
    <div className=' mt-10'>
    <h2 className=' text-2xl text-center mx-6 mt-10 md:text-3xl font-semibold'>{spot}</h2>
    <p className='mx-6 text-slate-400 text-center text-sm font-light mt-1 mb-6 sm:mb-14'>Explore {spot} in Your Own Backyard!</p>
     ...
     <div className="container mx-auto p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {places.map((feature, index) => (
            // feature.features.subFeatures.includes(spot) 
            feature.features.map(feat => feat.subFeatures.includes(spot))
            ? (
        <Link to={`/Spots`} key={index} onClick={() => {setSelectedSpot(feature.name), setAccountHolder(loginAccount)}}><Card src={feature.images} alt={feature.name} title={feature.name} address={feature.address} feature={spot} /></Link>
        ) : null
        ))} 
      </div>
    </div>
