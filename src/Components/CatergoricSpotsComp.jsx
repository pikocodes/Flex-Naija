...
function CatergoricSpotsComp({spot}) {
...
const places = [
    ...
    const [menuSpots, setMenuSpots] = useState([]);

    //switch the 'navSpots' to 'menuSpots' recieve data from server
    // useEffect(
    //     axios.get('URL Link')
    //     .then(resp => {

    //The requested catergory must match the catergory and location from the database before it renders the data that matches
    //{spotPlaceCategory} === resp.data.catergory === resp.data.location ? setMenuSpots(resp.data) : null;
    //     })
    //     .catch(err => {
    //         console.log(err)
    //     })
     ...
     <div className="container mx-auto p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {places.map((navSpot, index) => (
            spot === navSpot.category ? (
        <Link to={`/Spots`} key={index} onClick={() => setSelectedSpot(navSpot.name)}><Card src={navSpot.images[2]} alt={navSpot.name} title={navSpot.name} address={navSpot.address} feature={spot} /></Link>
        ) : null
        ))} 
      </div>
    </div>
