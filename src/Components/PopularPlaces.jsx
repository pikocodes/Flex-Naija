...
function PopularPlaces({loginAccount}) {
...
const places = [
    ...
    const [loungeVendors, setLoungeVendors] = useState([]);

    const { setSelectedSpot } = useSpotContext();
    const { setAccountHolder } = useAccountContext();

    //switch the 'popularPlaces' to 'loungeVendors' recieve data from server
    // useEffect(
    //     axios.get('URL Link')
    //     .then(resp => {
    //         setLoungeVendors(resp.data)
    //     })
    //     .catch(err => {
    //         console.log(err)
    //     })
  ...
  return (
    ...
    <div className=' mt-20'>
        ...
        <h2 className=' text-2xl text-center mx-6 mt-10 md:text-3xl font-semibold'>Popular Places</h2>
        <p className='mx-6 text-slate-400 text-center text-sm font-light mt-1 mb-6 sm:mb-14'>Explore Well-Known Destinations in Your Own Backyard!</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {places.map((popularPlace, index) => (
        <Link to={`/Spots`} key={index} onClick={() => {setSelectedSpot(popularPlace.name), setAccountHolder(loginAccount)}}><Card src={popularPlace.images[0]} alt={popularPlace.name} title={popularPlace.name} address={popularPlace.address} feature={popularPlace.category} /></Link>
        ))} 
      </div>
    </div>
